...
export default function GamesPage() {
  ...
  const [hasNotification] = useState(true) // placeholder: troca por lógica real futuramente
              ...
              ? (
                <div className={styles.userWidget}>
                  ...
                  <button className={styles.bellBtn} aria-label="Notificações">
                    <Bell size={18} />
                    {hasNotification && <span className={styles.bellDot} />}
                  </button>

                  {/* Card do usuário */}
                  <Link to="/perfil" className={styles.userCard} onClick={closeMenu}>
                    <img
                      src={playerData?.avatar_url || 'https://github.com/Melchior-Jr.png'}
                      alt="Avatar"
                      className={styles.userAvatar}
                    />
                    <div className={styles.userInfo}>
                      <span className={styles.userName}>
                        {playerData?.full_name?.split(' ')[0] || playerData?.username || 'Explorador'}
                      </span>
                      <span className={styles.userLevel}>NÍV. {Math.floor((playerStats?.galactic_xp || 0) / 1000) + 1}</span>
                    </div>
                  </Link>

                  {/* Logout */}
                  <button className={styles.logoutBtn} aria-label="Sair" onClick={handleLogout}>
                    <LogOut size={16} />
                  </button>
                </div>
              )
